...
class _MLKitCameraState extends State<MLKitCamera> {
  ...
  void initState() {
    ...
    cameraController.onNv21FrameCallback = (nv21) {
      debugPrint(
          "NV21 frame received: ${nv21.width}x${nv21.height}, size=${nv21.size}, fps=${nv21.fps}");
      setState(() {
        videoFramesReceived++;
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (_renderFps > 0)
                      _buildStatRow('GL Render FPS', _renderFps.toString(), highlight: true),
                    ...
                    _buildStatRow('Recording Status', isRecording ? 'Recording' : 'Not Recording',
  ...
  Widget _buildPoseDetectionInfo() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Poses detected: ${_detectedPoses.length}',
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold),
        ),
  ...
  List<Widget> _buildPoseDetailsList() {
    ...
    for (var i = 0; i < _detectedPoses.length; i++) {
      ...
      poseWidgets.add(
        Text(
          'Pose $i - ${landmarks.length} landmarks',
          style: TextStyle(fontSize: 12),
        ),
      );
      ...
      if (landmarks.containsKey(PoseLandmarkType.leftElbow) &&
