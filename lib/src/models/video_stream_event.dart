...
abstract class VideoStreamEvent {
  ...
  factory VideoStreamEvent.fromMap(Map<dynamic, dynamic> map) {
    ...
    switch (type) {
      ...
      case 'NV21':
        return Nv21FrameEvent.fromMap(map);
      case 'STATE':
...
/// NV21 原始帧事件（包含宽高）
class Nv21FrameEvent extends VideoStreamEvent {
  final Uint8List data;
  final int timestamp;
  final int size;
  final int fps;
  ...
  factory Nv21FrameEvent.fromMap(Map<dynamic, dynamic> map) {
    return Nv21FrameEvent(
      data: map['data'] as Uint8List,
      timestamp: map['timestamp'] as int,
      size: map['size'] as int,
      fps: map['fps'] as int,
